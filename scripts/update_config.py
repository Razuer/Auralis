#!/usr/bin/env python3
"""Update UI configuration values across theme files based on theme.toml."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Update UI configuration across theme files."
    )
    parser.add_argument(
        "--rounding", type=int, help="Set window rounding (0 = square, 10 = rounded)"
    )
    parser.add_argument(
        "--waybar-floating",
    ...
    parser.add_argument(
        "--skip-palette",
        action="store_true",
        help="Do not rewrite theme.toml with new values",
    )
    return parser.parse_args()


def update_palette_file(rounding: int | None, waybar_floating: bool | None) -> None:
    """Update theme.toml with new values."""
...
def main() -> None:
    ...
    if not args.skip_palette and (
        args.rounding is not None or args.waybar_floating is not None
    ):
